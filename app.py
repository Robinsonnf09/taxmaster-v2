...
# ============================================================================
# ROTAS DA CALCULADORA DE ATUALIZAÇÃO DE PRECATÓRIOS
# ============================================================================
...
def api_calculadora_calcular():
    """
    API para cálculo completo de atualização de precatório
    """
    try:
        data = request.json
        ...
        if not data.get('valor_original'):
            return jsonify({'error': 'Valor original é obrigatório'}), 400
        if not data.get('data_inicio'):
            return jsonify({'error': 'Data inicial é obrigatória'}), 400
        if not data.get('data_fim'):
            return jsonify({'error': 'Data final é obrigatória'}), 400
        
        # Parâmetros
        valor_original = float(data['valor_original'])
        data_inicio = data['data_inicio']
        data_fim = data['data_fim']
        indice = data.get('indice', 'IPCA-E')
        incluir_juros = data.get('incluir_juros_mora', True)
        taxa_juros = float(data.get('taxa_juros', 1.0))
        percentual_honorarios = float(data.get('percentual_honorarios', 0))
        
        # Calcular
        resultado = calculadora.calculo_completo(
            valor_original=valor_original,
            data_inicio=data_inicio,
            data_fim=data_fim,
            indice=indice,
            incluir_juros_mora=incluir_juros,
...
@app.route('/api/calculadora/indices-disponiveis', methods=['GET'])
def api_calculadora_indices():
    """
    Retorna lista de índices disponíveis para cálculo
    """
    indices = [
        {'id': 'IPCA-E', 'nome': 'IPCA-E', 'descricao': 'Índice de Preços ao Consumidor Amplo Especial'},
        {'id': 'INPC', 'nome': 'INPC', 'descricao': 'Índice Nacional de Preços ao Consumidor'},
        {'id': 'TR', 'nome': 'TR', 'descricao': 'Taxa Referencial'},
        {'id': 'SELIC', 'nome': 'SELIC', 'descricao': 'Sistema Especial de Liquidação e Custódia'},
        {'id': 'POUPANCA', 'nome': 'Poupança', 'descricao': 'Caderneta de Poupança'},
        {'id': 'IGP-DI', 'nome': 'IGP-DI', 'descricao': 'Índice Geral de Preços - Disponibilidade Interna'}
    ]
...
@app.route('/api/calculadora/simular-multiplos-indices', methods=['POST'])
def api_calculadora_simular_multiplos():
    """
    Simula cálculo com múltiplos índices para comparação
    """
    try:
        data = request.json
        ...
        indices = ['IPCA-E', 'INPC', 'TR', 'SELIC', 'POUPANCA', 'IGP-DI']
        resultados = {}
        ...
        for indice in indices:
            ...
            resultados[indice] = resultado['resumo']
